...
//
// These produce the exact shapes `lib/openproject/mappers.js` produces for
// the corresponding HAL responses. Every divergence is a UI bug waiting
// to happen, so when in doubt: copy the HAL mapper field for field.
...
// Mirrors `parseStoryPointValue` in lib/openproject/mappers.js.
...
export function mapSprintRow(r) {
  ...
  else if (status === "open" && (!startIso || startIso > todayIso)) state = "planned";
  else state = "active";
