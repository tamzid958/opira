// Server-only DB implementation of TaskRepository (reads only — Phase 1).
//
// Writes (create/update/delete) are intentionally not implemented here. The
// factory routes mutations to the API repo even when OPIRA_DATA_SOURCE=db,
// because OP's API generates journals, notifications, derived parent
// progress and webhook events — none of which a raw INSERT/UPDATE replicates.
//
// Story points: when OPENPROJECT_STORY_POINTS_FIELD is `storyPoints` (default)
// the value is read from `work_packages.story_points` directly. When it's
// `customFieldN`, we LEFT JOIN custom_values once (instead of two correlated
// subqueries) plus custom_options to resolve the option label. The row
// mapper picks label > value > parsed numeric.
...
const SP_FIELD = (process.env.OPENPROJECT_STORY_POINTS_FIELD || "storyPoints").trim();
