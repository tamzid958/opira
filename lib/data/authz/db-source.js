...
let nonMemberCache = null; // { perms: string[], publicProjectIds: number[], expiresAt: number }
...
async function loadNonMemberContext(pool) {
  if (nonMemberCache && nonMemberCache.expiresAt > Date.now()) {
    return nonMemberCache;
  }
 ...
 */
export async function fromDbPermissions(session) {
