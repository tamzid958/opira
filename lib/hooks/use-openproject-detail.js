...
export function useDocumentAttachments(docId, enabled = true) {
  return useQuery({
    queryKey: ["op", "document", docId, "attachments"],
    queryFn: () =>
      fetchJson(`/api/openproject/documents/${encodeURIComponent(docId)}/attachments`),
    ...enabledOpts(enabled && !!docId),
  });
}

export function useUploadDocumentAttachment(docId) {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async ({ file, description }) => {
      const fd = new FormData();
      ...
      if (description) fd.append("description", description);
      const res = await fetch(
        `/api/openproject/documents/${encodeURIComponent(docId)}/attachments`,
        { method: "POST", body: fd },
      );
      const body = await res.json().catch(() => ({}));
      if (!res.ok) {
    ...
    onMutate: async ({ file, description }) => {
      const key = ["op", "document", docId, "attachments"];
      await qc.cancelQueries({ queryKey: key });
      const prev = qc.getQueryData(key);
      const tempId = `tmp-att-${Date.now()}`;
      const optimistic = {
    ...
    onSettled: () =>
      qc.invalidateQueries({ queryKey: ["op", "document", docId, "attachments"] }),
  });
}

export function useDeleteDocumentAttachment(docId) {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (attachmentId) =>
      fetchJson(`/api/openproject/attachments/${encodeURIComponent(attachmentId)}`, {
