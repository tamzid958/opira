...
const ROLE_TTL_MS   = 60 * 60_000; // 1 hour in-process
const VIEWER_TTL_MS = 60 * 60_000; // 1 hour in-process (Redis L2 holds 1 day)

// makeCache gives bounded growth + TTL eviction; plain Map would accumulate
// stale entries forever on long-running servers.
const roleCache = makeCache({ ttlMs: ROLE_TTL_MS, maxEntries: 500 });
const viewerCache = new Map(); // userId -> { value, expiresAt } — managed manually for singleflight compat
...
async function fetchRolePermissions(roleId) {
  ...
  try {
    ...
    roleCache.set(roleId, mapped.permissions);
