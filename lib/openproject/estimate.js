...
const SCHEMA_MODE_CACHE = new Map();
const SCHEMA_MODE_TTL_MS = 10 * 60 * 1000;
...
export async function getProjectEstimateMode(projectId, sampleWp, opFetch) {
  ...
  if (cached && Date.now() - cached.t < SCHEMA_MODE_TTL_MS) return cached.value;
  ...
  SCHEMA_MODE_CACHE.set(cacheKey, { t: Date.now(), value: mode });
  return mode;
}
