...
export function mapProject(p) {
  ...
  return {
    id: identifier,
    nativeId: p.id ? String(p.id) : null,
...
// status (`open|locked|closed`) + start date → derived state used by the
// backlog/board flow chrome:
...
export function deriveSprintState(status, startIso, todayIso) {
  if (status === "closed") return "closed";
  if (status === "open" && (!startIso || startIso > todayIso)) return "planned";
  return "active";
}
...
export function mapVersionToSprint(v) {
  ...
  // (finished). Surface the raw status for UI badges + lock/unlock/reopen
  // actions; derive `state` for backlog/board flow chrome.
  const status = v.status || "open";
  const state = deriveSprintState(status, startIso, todayIso);
  return {
    id: String(v.id),
    name: v.name,
    state,
...
export function mapStatus(s) {
  return {
    ...
    defaultDoneRatio: typeof s.defaultDoneRatio === "number" ? s.defaultDoneRatio : null,
  };
}
...
export function parseStoryPointValue(raw) {
...
export function tshirtLabelForPoints(n) {
...
export function mapWorkPackage(wp, opts = {}) {
  ...
  return {
    ...
    statusDefaultDoneRatio: statusRecord ? (statusRecord.defaultDoneRatio ?? null) : null,
...
// ── GitHub pull requests (linked via OP's GitHub integration) ────────────
//
// Surfaced when the OP server has the GitHub plugin configured and a PR
// description / commit references the WP id. Read-only — OP doesn't expose
// PR mutations through v3. The check-run rollup follows GitHub's own
// precedence: any failure beats pending; any pending beats success.
function rollupCheckStatus(checkRuns) {
...
export function mapGithubPullRequest(pr) {
  ...
  const checkRuns = embedded.githubCheckRuns?._embedded?.elements
    || embedded.githubCheckRuns?.elements
    || embedded.githubCheckRuns
    || [];
  const merged = !!pr.merged;
  const draft = !!pr.draft;
  // OP/GitHub `state` is open|closed; merged is a closed-with-merge.
  // Collapse to a single display value the UI can switch on.
  const rawState = pr.state || null;
  const displayState = merged ? "merged" : draft ? "draft" : rawState;
  return {
    id: String(pr.id),
    ...
    draft,
    merged,
    htmlUrl: pr.htmlUrl || links.htmlUrl?.href || null,
    repositoryFullName: pr.repository || linkTitle(links.repository) || null,
    authorLogin: linkTitle(links.author) || null,
    authorAvatarUrl: links.author?.avatarUrl || null,
    githubUpdatedAt: pr.githubUpdatedAt || null,
    checksStatus: rollupCheckStatus(Array.isArray(checkRuns) ? checkRuns : []),
  };
}
...
export function mapDocument(d) {
  ...
  const embeddedAttachments =
    d._embedded?.attachments?._embedded?.elements ||
    d._embedded?.attachments?.elements ||
    [];
  return {
    id: String(d.id),
    ...
    attachments: embeddedAttachments.map(mapAttachment),
    permissions: {
